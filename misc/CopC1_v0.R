...
print.CS_Comp <-function(aa,...)
{
  cat("Selection Completed:\n")
  print(aa$Selected)
  for(i in 1:length(aa$paras)){
    cat(paste("\n",class(aa$estCop[[i]])[1],"(",aa$paras[i],"). L2:",aa$L2distance[i],sep = ""))
  }
}
print.CS_Fail <-function(aa,...)
{
  cat("Selection Failed due to lack of non-censored data:")
  for(i in 1:length(aa$paras)){
    cat(paste("\n",class(aa$estCop[[i]])[1],"(",aa$paras[i],"). Likelihood:",aa$likelihoods[i],sep = ""))
  }
}
...
myrCopula.joeCopula = function(t,u,a){
  ...
  while(sum(eps>0.000001) & (sum(eps>0.000001)>len/100 | max(eps) > 0.001) & nloop<10){
    ...
    f1 = mydCopula.joeCopula(cbind(u,v),a)
    eps = abs(f)
    v = v - f/f1
    v[v<0] <- -v[v<0]
    eps[v<0] <- 1
...
#' @importFrom  stats qt
#' @usage genLatentY(cop="Gaussian",theta=0.5,N=100,MARGIN.inv=qnorm,mean=0,sd=1)
...
#' By default, \code{jumps} = c(1,rep(0,n-1)).
#' See 'Details'.
...
#' @details
#' \itemize{
#'  \item{\code{jumps}}{ a Nx1 vector indicating whether each response at time t is a new startig point.
#'  For simulated data, only the first response at t=1 is a starting point
#'  and all the following responses are stationary time series.
#'  Hence for simulated data, by default, jumps = c(1,rep(0,N-1)).
#'  For the water data, the measuring date varies.
#'  Therefore, if the time lag for response Y_{t} is larger than some threshod,
#'  we should treat Y_{t} and Y_{t-1} independently,
#'  otherwise the dependence (copula parameter) will be overestimated.
#'  In water data, column water$Indep should be used as input for jumps.}
#'  \item{\code{MARGIN,...}}{ the marginal distribution of the latent time series.
#'  The default is adjusted cdf:
#'  \eqn{\Sigma I_{Y_t<=y}/(n+1)}{\sum I_{Yt<=y}/(n+1)}.
#'  The marginal distribution can be specified as
#'  other univariate probability function with followed arguments.}
#' }
#' @return \code{estCopC} returns a list of components related to optimization results.
...
#' @examples
#' ### Using simulated data as demonstration:
#' set.seed(20)
...
#' estCopC(cop = "Clayton",Y,d,delta=rep(FALSE,length(Y)),interval = c(1,10)) ## Omniscient estimator
#' estCopC(cop = "Clayton",Yc,d,delta,nIS = 1000,MARGIN=pt,df=3,interval = c(1,10)) ## CopC estimator under true marginal
#' ### Using water data as demonstraion:
#' attach(water)
#' set.seed(1)
...
# 8. Copula selection
#' CopC based on copula selection
#'
...
#' @param cop.typ a Kx1 vector containing the candidate copulas,
#' where K = length(cop.type) is the number of candidate copulas.
#' There are currently five available copula funcitons,
#' including clayton copula, normal copula, gumbel copula, joe copula and frank copula.
#' Select each by specifying from vector
...
#' The default is 500.
#' @param jumps the Nx1 vector indicating whether
#' each response at time t is a starting point of a new time series,
#' and hence should be treated independently from previous response.
#' By default, jumps = c(1,rep(0,n-1)).
#' @param MARGIN the marginal distribution of the latent time series.
#' @param ... additional parameters for the marginal distribution of the latent time series.
#' @param intervals a 2xK matrix specifying
#' the lower and upper bound for the copula parameter of each candidate copula.
#' each column is a pair of lower and upper bound
#' corresponind to the candidate copula.
#' @return \code{selectCopC} returns a list of components related to copula selection results.
...
#' \item{L2distance}{a Kx1 vector containing the L2 distance of each copula
#' with estimated copula parameter
#' from the empirical copula function.}
#' \item{Selected}{The selected copula object, which is the closest to the empirical copula function.}
#' @examples
...
#' obtain the conditional tau'th quantile of Y_{n+1} given {Y1,...,Yn}.
...
#' @importFrom  msm rtnorm
#' @usage
#' condQestCopC(tau1=0.5,Yc,d,delta,copula=claytonCopula(1),cop=NULL,theta=NULL,nIS=10000,MARGIN=NULL,MARGIN.inv=NULL,...)
#' @param tau1 the conditional quantile level, a numeric value between 0 and 1.
#' @param Yc the Nx1 vector of observed response variable
#' that is subject to lower detection limit.
#' @param d the lower detection limit.
#' @param delta the Nx1 vector of censoring indicator
...
#' @param ... additional parameters for the marginal distribution of the latent time series.
#' @return \code{condQestCopC} returns the conditional tau'th quantile of Y_{n+1} given {Y1,...,Yn}
#' based on the specified copula funciton with its parameter and marginal distribution.
#' @references Li, F., Tang, Y. and Wang, H. (2018) Copula-based Semiparametric Analysis for Time Series Data with Detection Limits
#' @examples
#' set.seed(20)
...
#' @export
condQestCopC=function(tau1,Yc,d,delta,copula=NULL,cop=NULL,theta=NULL,nIS=10000,MARGIN=NULL,MARGIN.inv=NULL,...)
{
  if(!is(copula,'Copula')){
  ...
  return(cqCopC(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...))
}

cqCopC <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  UseMethod("cqCopC",copula)
}
cqCopC.normalCopula <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  ...
  if(clen==0){
    ...
    ff1 <- function(q){
      ...
      return(numer/denum-tau1)
    }
    if(ff1(Pi,nIS)>=0){
      qq <- Pi
    }else{
      solvethis <- try(uniroot(ff1,interval = c(Pi,1-1e-6)),silent = T)
      psolve <- ifelse(is.character(solvethis),1,solvethis$root)
      qq <- psolve
    }
  }
  if(qq==1){
...
cqCopC.claytonCopula <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  theta = copula@parameters
  N = length(Yc)
  clen = ifelse(delta[N],tail(rle(delta)$length,1),0)
  ...
  u0 = MARGIN(tail(Yc,clen+1)[1],...)
  if(clen==0){
    vv = myrCopula.claytonCopula(tau1,u0,theta)
    qq = vv
    qq = pmax(Pi,vv)
  }else{
    ff1 <- function(q){
...
cqCopC.gumbelCopula <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  theta = copula@parameters
  N = length(Yc)
  clen = ifelse(delta[N],tail(rle(delta)$length,1),0)
  ...
  u0 = MARGIN(tail(Yc,clen+1)[1],...)
  if(clen==0){
    vv = myrCopula.gumbelCopula(tau1,u0,theta)
    qq = vv
    qq = pmax(Pi,vv)
  }else{
    ff1 <- function(q){
...
cqCopC.frankCopula <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  theta = copula@parameters
  N = length(Yc)
  clen = ifelse(delta[N],tail(rle(delta)$length,1),0)
  ...
  u0 = MARGIN(tail(Yc,clen+1)[1],...)
  if(clen==0){
    vv = myrCopula.frankCopula(tau1,u0,theta)
    qq = vv
    qq = pmax(Pi,vv)
  }else{
    ff1 <- function(q){
...
cqCopC.joeCopula <- function(tau1,Yc,d,delta,nIS,copula,MARGIN,MARGIN.inv,...)
{
  theta = copula@parameters
  N = length(Yc)
  clen = ifelse(delta[N],tail(rle(delta)$length,1),0)
  ...
  u0 = MARGIN(tail(Yc,clen+1)[1],...)
  if(clen==0){
    vv = myrCopula.joeCopula(tau1,u0,theta)
    qq = vv
    qq = pmax(Pi,vv)
  }else{
    ff1 <- function(q){
